...
| `LOG_FILE_PATH` (or similar) | B | Log file location | Optional; default e.g. `logs/app.log` relative to project root. |
...
- **Create** `src/lib/system-messages.ts`.
  - In-memory store holding the last N messages (e.g. 50–100). Each entry: `{ id: string, level: 'error' | 'warning' | 'info' | 'debug', timestamp: string (ISO), message: string, stack?: string, context?: Record<string, unknown>, route?: string }`.
  - Export a function to **add** a message (and optionally a **getter** that accepts an optional level filter, e.g. `getMessages(level?: 'error' | 'warning' | 'info' | 'debug')` or multiple levels, returning the list, newest first).
  - When `report(level, messageOrError, context)` is called, push to this store (and write to the log file). Same process-only guarantee as sync-status; persisting to DB is an optional follow-up.
...
- **System-message store:** Bounded list (e.g. 50–100 entries). When full, drop oldest when appending. Newest first when returning.
