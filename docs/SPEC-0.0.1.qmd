...
## Questions: Filesystem-Based Storage

### Rationale
Questions are stored in the **filesystem**, not in the database, so that:
- Users can develop and version questions on their own machine (e.g. with Git).
...
- **Questions**: Filesystem under a configured root (e.g. in repo or mounted volume); format and metadata as in QUESTION-FORMAT-0.0.1.qmd.
...
- **QUESTION-FORMAT-0.0.1.qmd** — Question filesystem layout and metadata schema (see next section).
