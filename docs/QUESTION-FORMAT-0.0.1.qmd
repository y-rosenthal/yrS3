...
- **New question**: Author or user provides content (via UI or a folder/zip). System creates a new `logical_id` (e.g. UUID), assigns version `1.0.0.0`, and writes files under `questions/{logical_id}/{version}/`. No overwrite of existing version folders.
- **New version**: Author or owner provides updated content and a **version bump type** (question_major, question_minor, answer_major, answer_minor). System computes the next version string, writes a **new** folder `questions/{logical_id}/{new_version}/`, and does not modify any existing version folder. Only the owner (from the database) may create a new version for a given logical_id.
