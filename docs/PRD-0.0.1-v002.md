...
YRTODO: Finalize prompt templates and constraints to guarantee deterministic AI grading.
...
YRTODO: Decide on sandbox implementation strategy (e.g., container-based vs WASM vs managed service).
...
YRTODO: Specify sandbox security constraints in detail  
YRTODO: Define evaluator output schema (score, feedback, diagnostics)  
YRTODO: Decide on logging and error-handling strategy  
YRTODO: Define initial test authoring workflow (admin-only vs UI-based)
