...
- `QUESTION_SYNC_OWNER_ID` — when using FS for questions, set to a user UUID so that FS-only question versions are imported into the DB when the app first lists questions. See SPEC-DB-FS-QUESTION-SYNC-0.0.3.qmd.
...
| `QUESTION_SYNC_OWNER_ID` | No | — | When using FS for questions: UUID of user to assign when importing FS-only versions into the DB (required for sync import). See SPEC-DB-FS-QUESTION-SYNC. |
