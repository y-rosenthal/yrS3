...
2. **Where confirmation emails go (local):**  
   Supabase has replaced **Inbucket** with **Mailpit** for local email testing; the local stack uses Mailpit to capture outgoing mail. In `config.toml`, the email testing server is still configured under the **`[inbucket]`** section—Supabase kept the same section name for backward compatibility, so existing configs and CLI tooling continue to work. After `supabase start`, run:
   ```bash
   supabase status
   ```
