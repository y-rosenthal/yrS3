...
## Installing required software (Ubuntu / Plasma KDE)
...
### Vercel CLI (optional, for deploy from the terminal)
...
Use `supabase stop --no-backup` to stop the stack and **discard** local database and other local data (no backup is kept). If you are **not** using the local Supabase stack and instead use a **hosted** Supabase project for day-to-day development, skip Section 1.6 and go to **Section 2.3** (Supabase project setup) to link and use your hosted project.
...
### Create questions directory (filesystem mode)
...
### Separate dev, test, and production environments (hosted Supabase)
