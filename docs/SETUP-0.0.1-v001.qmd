...
## Quick setup (after a new release) {#sec-quick-setup}

If you are already developing the system and have your environment set up, use this checklist when you pull or receive a new release. You do **not** need to reinstall Node, Docker, or the Supabase CLI unless a release explicitly requires a newer version.

1. **Get the latest code** — e.g. `git pull`.
2. **Install or update dependencies** — `npm install`.
3. **Database**
   - **Local Supabase:** If the release includes new migrations or seed changes, run `supabase db reset` to apply migrations and re-run the seed (this wipes local DB data). To apply only new migrations without wiping, use `supabase db push`.
   - **Hosted Supabase:** When there are new migrations, run `supabase db push` from the project root (with the project linked).
4. **Environment** — Ensure `.env.local` exists and has any new variables; compare with `.env.example` and add or update keys as needed (see @sec-environment-variables).
5. **Start the dev server** — `npm run dev`.
6. Open [http://localhost:3000](http://localhost:3000).

If you use **local Supabase**, start it before the dev server: `supabase start` (and `supabase stop` when you are done). For full first-time setup or when you change machine or stack, see the sections below.
...
- `QUESTION_SETS_ROOT` — directory for file-based question sets (default: `./question-sets`). See QUESTION-SET-FORMAT-0.0.2.qmd.
- `QUESTION_SYNC_OWNER_ID` — optional when using the seeded default question owner (e.g. after `supabase db reset`); if set, overrides the default. For out-of-the-box FS question import, run `supabase db reset` and copy `supabase status -o env` into `.env.local`. Default question owner for local login: email `default-question-owner@system.local`, password `default-question-owner-password`. See SPEC-DB-FS-QUESTION-SYNC-0.0.3.qmd.
- `QUESTIONS_FS_BACKUP_ROOT` — when `QUESTIONS_STORAGE=supabase`, optional local path to mirror each version for backup.

### Supabase project setup (CLI-first where possible) {#sec-supabase-project-setup}

If you prefer a **fully local** backend (including auth), see @sec-running-supabase-locally. Otherwise, you need a **hosted** Supabase project for auth and (optionally) question storage. The CLI cannot create a new project; that is done once in the Dashboard. After that, use the CLI for linking and migrations. For **separate dev, test, and production** environments using multiple hosted projects (no Docker), see @sec-separate-environments.
...
#### (Any user can create questions; no author setup required)


(Apply migrations via Dashboard SQL Editor or `supabase db push`; no author table.)
...
Question sets are stored in the database (and optionally on the filesystem). After running migrations, a seed may create one sample set; or create sets via the UI at **Question sets** → **Create question set**, or add folders under `question-sets/` with a `set.yaml` per set (see QUESTION-SET-FORMAT-0.0.2.qmd).
...
- **Question sets**: use "Question sets" to list sets, then "Take as test" on a set to answer and submit. Create sets via "Create question set".
- **Questions**: use "My questions" or "Upload questions (files)" to create and manage questions. Any user can propose a new version; the version is pending until the question owner approves it.
...
vercel env add QUESTION_SETS_ROOT production
...
2. **Project** → **Settings** → **Environment Variables**. Add:
   ...
   - Optionally `QUESTIONS_BUCKET`, `QUESTION_SETS_ROOT`, `QUESTION_SYNC_OWNER_ID`, `QUESTIONS_FS_BACKUP_ROOT`, `NEXT_PUBLIC_APP_URL`
3. Deploy (automatic on push, or **Deployments** → **Redeploy**).

### Questions on Vercel

With `QUESTIONS_STORAGE=supabase`, question files are read/written to the Supabase Storage bucket created by the migration. Users upload via the app; the app does not use a local `questions` folder in production.
...
**5. (Optional) Verify question permissions**

Any authenticated user can create questions and propose new versions. No separate role or table is required.
...
| `QUESTION_SYNC_OWNER_ID` | No | (seeded default) | When using FS for questions: UUID of user for FS-only import. Optional after `supabase db reset` (seed creates default question owner). If set, overrides the default. See SPEC-DB-FS-QUESTION-SYNC. |
| `QUESTIONS_FS_BACKUP_ROOT` | No | — | When `QUESTIONS_STORAGE=supabase`: local path to mirror each question version for backup (content + db_meta.yaml). |
| `NEXT_PUBLIC_APP_URL` | No | — | App URL for redirects |
| `LOG_FILE_PATH` | No | `logs/app.log` | System log file path (SPEC 0.0.5). |
| `SYSTEM_MESSAGE_STORE_SIZE` | No | 1000 | Max system messages kept in memory; older messages load from log file. |
| `SYSTEM_MESSAGE_LOAD_OLDER_BATCH` | No | 100 | Number of older messages loaded per "Load older" on /logs page. |
| `NEXT_PUBLIC_LOGS_REFRESH_INTERVAL_MS` | No | 4000 | System log page auto-refresh interval (ms). |

See `.env.example` in the repo root for full comments on all options.
...
- **QUESTION-FORMAT-0.0.1.qmd** — Question folder layout, validation rules, and optional db_meta.yaml
